...
object TopN {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local[2]").appName("TopN")
      .config("spark.sql.sources.partitionColumnTypeInference.enabled", false)
      .getOrCreate()
    val df = spark.read.format("parquet").load("E:/ImoocData/clean")
    // DataFrame表形式：  http://www.imooc.com/article/17891  article  17891  407  218.75.35.226  北京  08:07:35  2017-05-11
    df.printSchema()
    df.show(false)
